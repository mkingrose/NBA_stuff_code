// buiit with guidance from https://observablehq.com/
export default function define(runtime, observer) {
    const main = runtime.module();
    const fileAttachments = new Map([["Resources/money4.csv"]]);
    main.builtin("FileAttachment", runtime.fileAttachments(name => fileAttachments.get(name)));
    main.variable(observer()).define(["md"], function (md) {
    
    main.variable(observer("data")).define("data", ["FileAttachment"], function (FileAttachment) {
        return (
            FileAttachment("Resources/money4.csv").csv({ typed: true })
        )
    });
                ...
                return ([date, data], transition) => label = label
                    ...
                    .join(
                        enter => enter.append("text")
                            .attr("transform", d => `translate(${ x((prev.get(d) || d).value)
    }, ${ y((prev.get(d) || d).rank)
})`)
